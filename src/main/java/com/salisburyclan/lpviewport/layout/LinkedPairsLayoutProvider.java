...
public class LinkedPairsLayoutProvider implements LayoutProvider {
  ...
  private final static String TYPE = "linked";
  ...
  public ListenableFuture<Viewport> createLayout(String layoutSpec, Collection<LaunchpadDevice> devices) {
    ...
    if (viewports.isEmpty()) {
      ...
      return makeLinkedViewport(devices.stream()
          .map(LaunchpadDevice::getViewport)
          .collect(Collectors.toList()));
    }
  }
  ...
  private static class ViewportBuilder {
    ...
    private static class Link {
    	...
    	public String toString() {
    		return String.format("Link(%s, %s, (%d, %d), %s, (%d, %d))",
    ...
    private void selectLinkStart(Viewport startViewport, int startX, int startY) {
    	...
    	unselectedViewports.forEach(viewport -> {
