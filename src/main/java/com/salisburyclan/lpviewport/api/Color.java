...
public final class Color {
    ...
    public static Color of(int red, int green, int blue) {
        if (red < MIN_INTENSITY || red > MAX_INTENSITY) {
            throw new IllegalArgumentException("Invalid red value : " + red + ". Acceptable values are in range [0..63].");
        }
        if (green < MIN_INTENSITY || green > MAX_INTENSITY) {
            throw new IllegalArgumentException("Invalid green value : " + green + ". Acceptable values are in range [0..63].");
        }
        if (blue < MIN_INTENSITY || blue > MAX_INTENSITY) {
            throw new IllegalArgumentException("Invalid blue value : " + blue + ". Acceptable values are in range [0..63].");
        }
